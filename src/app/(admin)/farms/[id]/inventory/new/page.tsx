...
export default function CreateInventoryItemPage() {
  ...
  const { addInventoryItem, clearError } = useInventory();
  ...
  const handleSubmit = useCallback(
    async (data: CreateInventoryItemRequest) => {
      try {
        await addInventoryItem(farmId, data);
        addNotification({
          type: 'success',
          title: 'Item Created',
        ...
        addNotification({
          ...
          message: error || 'Failed to create inventory item. Please try again.',
        });
      }
    },
    [addInventoryItem, farmId, router, addNotification]
  );
  ...
  return (
    <div className="max-w-3xl mx-auto w-full p-6 space-y-6">
      ...
      <div className="flex flex-col gap-2">
        <h1 className="text-xl font-semibold text-gray-900 dark:text-white">Create Inventory Item</h1>
        <p className="text-sm text-gray-600 dark:text-gray-400">
          Add a new item to track in your farm inventory.
        </p>
      </div>
