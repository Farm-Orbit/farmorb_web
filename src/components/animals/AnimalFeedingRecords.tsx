...
export default function AnimalFeedingRecords({ farmId, animalId }: AnimalFeedingRecordsProps) {
  ...
  }, [farmId, animalId, getFeedingRecords]);
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-end">
        <Button
          size="sm"
          onClick={() => router.push(`/farms/${farmId}/feeding/records/new?animal_id=${animalId}`)}
          data-testid="animal-feeding-create-button"
        >
          Add Feeding Record
        </Button>
      </div>
      ...
      {records.length === 0 ? (
        <div
          ...
          data-testid="animal-feeding-records-empty"
        >
          <h3 className="text-lg font-medium text-gray-600 dark:text-gray-400">No feeding records</h3>
          <p className="text-sm text-gray-500 dark:text-gray-500 mt-2">
            Feeding records for this animal will appear here.
          </p>
        </div>
      ) : (
        <div data-testid="animal-feeding-records-table">
          <CustomMaterialTable
            columns={columns}
            data={records}
            isLoading={isLoading}
